...
#for now assume that I am only using the functional API, with inputs,outputs,and name provided
class MetricsModel(keras.Model):
    def __init__(self, inputs, outputs, name):
        super(MetricsModel, self).__init__(inputs=inputs, outputs=outputs, name=name)
        
    #Taken from github 8/23/22
    #just added stuff to return
    def train_step(self, data):
        ...
        return self.compute_metrics(x, y, y_pred, sample_weight, tape)
        
    def compute_metrics(x, y, y_pred, sample_weight, tape):
        pass
...
def binary_output_fn(output_size):
    return layers.Dense(output_size, activation="sigmoid", name="BinaryOutput")
...
def gcn_convolution_fn(node_features, adjacency_matrix, params):
    ...
    transformed_node_features = layers.Dense(
        hidden_size, activation="elu", kernel_initializer='he_normal', name="Transform")(node_features)
